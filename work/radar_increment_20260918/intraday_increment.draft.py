...
def load_previous_close(day):
    ...
    result = read_archived_strategy_result(previous, "macd-j-upturn-main-board", strategy_version="v1")
    if result is None:
...
def with_previous_close_difference(scan, *, baselines=None):
    ...
    if (scan.get("scan_kind") in {"manual_close", "close_confirmation"}
    ...
    if old_ids is None:
        ...
        result.update(status="baseline_unavailable", scan_candidates=[], records=[],
                      message="上一交易日正式 MACD＋KDJ 存档不可用，暂时无法判断新增；原始扫描已保留。")
        return result
