...
if args.action == 'archive':
    assert not active, 'Active jobs must finish before exclusive archive migration'
    ...
    with exclusive_worker_lock(), AnalysisJobStore() as jobs:
        ...
        try:
            ...
            baseline = runtime.selection_store.get_strategy_result(snapshot.trade_date,'macd-j-upturn-main-board',strategy_version='v2')
            assert baseline and baseline.payload.matched_count == 243
...
if args.action == 'scan':
    ...
    assert raw.get('screen_version') == 'macd-j-upturn-main-board.v2'
