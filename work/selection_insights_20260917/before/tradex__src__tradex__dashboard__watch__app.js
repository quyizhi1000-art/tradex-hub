(() => {
  ...
  function selectIntradayMacdJScan(index) {
    ...
    const candidates = asArray(scan.scan_candidates);
    ...
    ["股票", isClose ? "收盘价" : "参考价", "信号分组", "金叉位置", "J 拐头与低位标签", "指标证据", "数据时间"].forEach(label => {
      const cell = createElement("th", "", label);
      cell.scope = "col";
      header.append(cell);
    });
    ...
    candidates.forEach(record => {
      ...
      row.append(name, createElement("td", "", formatLevel(record.price)),
