...
(async()=>{
  ...
  assert.equal(watch.comparison.new_count,expected.length);
  assert.equal(watch.screen_version,"macd-j-upturn-main-board.v3");
  assert.equal(watch.comparison.baseline.strategy_version,"v3");
  assert.ok(watch.scan_candidates.every(c=>c.macd_cross_date && c.macd_cross_age_sessions>=0 && c.macd_cross_age_sessions<=2));
  assert.equal(watch.comparison.removed_count,raw.scan_candidates.length-expected.length);
  ...
  try{
    ...
    assert.equal(await page.locator('#intraday-macd-j-history-result tbody tr').count(),expected.length);
    assert.match(await page.locator('#intraday-macd-j-history-result').innerText(),new RegExp(`本轮新增 ${expected.length} 只`));
    const recentExample=watch.scan_candidates.find(c=>c.macd_cross_age_sessions===2 && c.j_turn_date===watch.trade_date);
    assert.ok(recentExample);
    const exampleRow=await page.locator('#intraday-macd-j-history-result tbody tr').filter({hasText:recentExample.instrument_id}).innerText();
    assert.ok(exampleRow.includes(recentExample.macd_cross_date));
    assert.ok(exampleRow.includes('距扫描日 2 个交易日'));
    receipt.confirmedExample={instrument:recentExample.instrument_id,crossDate:recentExample.macd_cross_date,jTurnDate:recentExample.j_turn_date};
    await page.evaluate(()=>{
    ...
    await page.locator('#intraday-macd-j-history-dialog').screenshot({path:path.join(__dirname,'radar-recent-cross.png')});
    ...
    await page.waitForFunction(()=>document.getElementById('stock-macd-j-rule').textContent.includes('v3') &&
      document.querySelectorAll('#stock-macd-j-table-body tr').length===105);
    ...
    await page.locator('#stock-selection-panel-macd-j').screenshot({path:path.join(__dirname,'daily-recent-cross.png')});
