...
(async()=>{
  ...
  const baseline=JSON.parse(fs.readFileSync(path.join(__dirname,'baseline-original.json'),'utf8'));
  ...
  assert.equal(watch.screen_version,"macd-j-upturn-main-board.v1");
  assert.equal(watch.comparison.baseline.strategy_version,"v1");
  ...
  try{
    ...
    await page.locator('#intraday-macd-j-history-dialog').screenshot({path:path.join(__dirname,'radar-restored.png')});
    ...
    await page.waitForFunction(()=>document.getElementById('stock-macd-j-rule').textContent.includes('当天新金叉') &&
      document.querySelectorAll('#stock-macd-j-table-body tr').length===56);
    ...
    await page.locator('#stock-selection-panel-macd-j').screenshot({path:path.join(__dirname,'daily-restored.png')});
