...
def load_previous_close(day):
    ...
    result = read_archived_strategy_result(previous, "macd-j-upturn-main-board", strategy_version="v2")
    if result is None:
    ...
    if (result.trade_date != previous or result.quality == "unavailable"
            ...
            or screen.screen_version != "macd-j-upturn-main-board.v2"):
...
def with_previous_close_difference(scan, *, baselines=None):
    ...
    if (scan.get("screen_version") != "macd-j-upturn-main-board.v2"
    ...
    if day not in baselines:
        baselines[day] = load_previous_close(day)
    old_ids, baseline = baselines[day]
