...
(() => {
  ...
  if (!panel) return;
  const themeNames = { paper: "暖纸米白", mist: "雾蓝浅灰", sage: "鼠尾草绿", graphite: "柔和石墨", classic: "经典深青" };
  ...
  panel.addEventListener("click", event => {
    const button = event.target.closest("[data-appearance-theme], [data-appearance-layout]");
    if (!button || !panel.contains(button)) return;
    const { appearanceTheme: theme, appearanceLayout: layout } = button.dataset;
    ...
    try {
      localStorage.setItem(storageKey, JSON.stringify({ theme: root.dataset.theme, layout: root.dataset.layout }));
...
(() => {
  ...
  function renderIntradayMacdJ(payload) {
    ...
    const fresh = (payload.status === "monitoring" || closeScan) && payload.trade_date === shanghaiClock().date;
    ...
    if (newAlerts.length) {
      ...
      byId("intraday-macd-j-toast-text").textContent = `${closeScan ? "手动收盘扫描（预估）" : "MACD + J 线盘中"}命中 ${newAlerts.length} 只：${newAlerts.slice(0, 5).map(r => `${r.name} ${r.instrument_id}`).join("、")}${newAlerts.length > 5 ? "等，完整名单见扫描记录" : ""}。${closeScan ? "使用收盘价推算，等待正式指标确认。" : "收盘前可能失效。"}`;
  ...
  async function fetchIntradayMacdJ() {
    ...
    try {
      ...
      if (byId("intraday-macd-j-history-dialog").open) fetchIntradayMacdJHistory(state.intradayMacdJHistoryDate);
    } catch (_) {
  ...
  function selectIntradayMacdJScan(index) {
    ...
    target.append(createElement("p", "", ["monitoring", "close_scanned", "close_confirmed"].includes(scan.status)
    ...
    visibleCandidates.forEach(record => {
      ...
      row.append(name, createElement("td", "", stockSelectionIndustryName(record)),
        ...
        createElement("td", "", isClose ? `${record.price_trade_date} 收盘日线` : formatTimestamp(record.observed_at, true)));
      body.append(row);
    });
    table.append(head, body);
    scroll.append(table);
  ...
  async function fetchIntradayMacdJHistory(day = null) {
    ...
    byId("intraday-macd-j-history-tabs").replaceChildren();
    byId("intraday-macd-j-history-result").replaceChildren();
    try {
      ...
      state.intradayMacdJScans.forEach((scan, index) => {
        const button = createElement("button", "", `${scan.last_scan_slot.slice(11, 16)}${scan.scan_kind === "close_confirmation" ? " 收盘确认" : scan.scan_kind === "manual_close" ? " 收盘预估" : ""} · ${["monitoring", "close_scanned", "close_confirmed"].includes(scan.status) ? asArray(scan.scan_candidates).length + " 只" : "未完成"}`);
        button.type = "button";
