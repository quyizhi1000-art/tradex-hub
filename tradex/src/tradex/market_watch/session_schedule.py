...
The closing call auction is one market event, not three ordinary continuous-
trading minutes.  Market-watch therefore samples continuous trading through
14:56 and requires one verified final-close observation at 15:00.
"""
...
EXPECTED_MARKET_WATCH_MINUTES = 238
FINAL_CLOSE_TIME = time(15, 0)


def expected_market_watch_minutes(trade_date: date) -> tuple[datetime, ...]:
    """Return continuous observations plus one final closing-auction result."""
...
def is_continuous_market_watch_minute(value: datetime) -> bool:
    """Return whether a stale heartbeat may represent this continuous minute."""

    if value.tzinfo is None or value.utcoffset() is None:
        raise ValueError("market-watch minute must include a timezone")
    local = value.astimezone(SHANGHAI)
    clock = local.time().replace(tzinfo=None)
    return time(9, 30) <= clock < time(11, 30) or time(13, 0) <= clock < time(14, 57)
