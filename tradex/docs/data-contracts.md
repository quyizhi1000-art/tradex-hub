...
`stock_relationship_profile.v1`。展示字段包括真实主营、业务标签、申万三级统计行业、
...
`unmatched_count`。分类页签只按 `primary_business_key` 生成；已有目录记录但主营尚未
核验、或目录没有该股票时，都进入“主营待核验”。目录缺失、部分股票未匹配、主营未
分类或规范涨停状态降级时，`quality=degraded` 并保留明确 `quality_flags`。
...
半导体材料。“销售商品”“合计特别调整”“分部间抵销”等会计汇总项也不能成为主营
标签。电子分销、贵金属、纺织、氟化工、中成药和医药等容易跨界的行业节点，必须有
当期主营分部支撑；只有旧版公司简介命中时不能自动升级。对于分销与产品、传统与转型业务并存的高冲突公司，以最新正式披露和最新
...
归类并展示业务标签与独立统计行业；复盘观察股同时显示“主营 / 观察方向”；每日选股使用申万三级
